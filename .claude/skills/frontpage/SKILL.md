...
| Stage | Turns | Input Tokens | Output Tokens | Model | Cost |
|-------|-------|-------------|---------------|-------|------|
| 1. Import & Strip | ~12 | 580K | 30K | Haiku | $2 |
| 2. Localize & Serve | ~49 | 2.97M | 155K | Opus | $13 |
| 3. Rebuild as Components | ~102 | 6.25M | 470K | Cheap + Opus | ~$8 |
| **Total** | **~163** | **~9.8M** | **~655K** | | **~$23** |

Stage 3 uses **chunk-and-stitch** (see [rebuild.md](./rebuild.md)): extract line ranges from `index.html` with `sed -n 'X,Yp'`, prepend a 3-line prompt, send to a cheap model. One section = one call. Opus only for interactive sections (navbar, animations). Skip Stage 3 entirely if the static serve works and you don't need to modify content — Stages 1-2 cost ~$15.
