...
declare -a CHECKS=(
    ...
    "vitest|Vitest setup validation|check-vitest-setup.sh|true"
    "vitest-versions|Vitest version pinning|check-vitest-versions.sh|true"
    "test-only|No focused tests (.only)|check-no-test-only.sh|true"
