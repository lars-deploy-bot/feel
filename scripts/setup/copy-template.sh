...
if [ -f "$ENV_FILE" ]; then
    ...
    # Check if ALIVE_ENV is set correctly
    if ! grep -q "^ALIVE_ENV=local" "$ENV_FILE" 2>/dev/null; then
        echo -e "${YELLOW}⚠${NC} ALIVE_ENV not set to 'local' in .env.local"
    ...
    if [ "$ENV_NEEDS_UPDATE" = true ]; then
        ...
        echo -e ""
        echo -e "     ${GREEN}ALIVE_ENV=local${NC}"
