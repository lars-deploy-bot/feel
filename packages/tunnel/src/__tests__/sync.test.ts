/**
 * Unit tests for sync.ts — Caddyfile generation (pure function),
 * hostname validation (standalone predicate), and site count guard.
 */

import type { InfrastructureService } from "@webalive/shared"
import { describe, expect, it } from "vitest"
import { generateCaddyInternal, HOSTNAME_REGEX, isValidHostname, MIN_EXPECTED_SITES } from "../sync.js"

/** Minimal infra services fixture using the shared type */
const TEST_INFRA: readonly InfrastructureService[] = [
  { subdomain: "widget", port: 5050, routeVia: "direct" as const, displayName: "Widget" },
  { subdomain: "dl1", port: 8444, routeVia: "caddy" as const, displayName: "E2E Test" },
]
...
// ---------------------------------------------------------------------------
// generateCaddyInternal — pure function, no side effects
// ---------------------------------------------------------------------------
...
describe("generateCaddyInternal", () => {
  it("generates valid Caddy map syntax with sites", () => {
    ...
    const output = generateCaddyInternal(sites, [], 3888, TEST_BASE_DOMAIN, TEST_PATHS)
    ...
    expect(output).toContain(`*.${TEST_BASE_DOMAIN} "localhost:3888"`)
    expect(output).toContain('default "localhost:3888"')
  ...
  it("skips invalid hostnames and reports them via callback", () => {
    const sites = new Map([
    ...
    const output = generateCaddyInternal(sites, [], 3888, TEST_BASE_DOMAIN, TEST_PATHS, h => skipped.push(h))
  ...
  it("handles empty sites map", () => {
    const output = generateCaddyInternal(new Map(), [], 3888, TEST_BASE_DOMAIN, TEST_PATHS)
    expect(output).toContain(":8444 {")
    expect(output).toContain(`*.${TEST_BASE_DOMAIN} "localhost:3888"`)
    expect(output).toContain('default "localhost:3888"')
  })
  ...
  it("includes direct infra services in map for wildcard fallback", () => {
    const output = generateCaddyInternal(new Map(), TEST_INFRA, 3888, TEST_BASE_DOMAIN, TEST_PATHS)
    ...
    expect(output).not.toContain('dl1.alive.best "localhost:8444"')
  })
  ...
  it("uses baseDomain for wildcard entry (not hardcoded)", () => {
    const output = generateCaddyInternal(new Map(), [], 3888, "sonno.tech", TEST_PATHS)
    expect(output).toContain('*.sonno.tech "localhost:3888"')
