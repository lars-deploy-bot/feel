...
/**
 * Generate the content of Caddyfile.internal from site domains + infrastructure services.
 *
 * This is the ONLY way Caddyfile.internal should be produced — never edit it manually.
 */
function generateCaddyInternal(sites: Map<string, number>, previewProxyPort: number): string {
  const lines: string[] = [
  ...
  for (const [hostname, port] of sortedSites) {
    if (!isValidHostname(hostname)) {
      log(`WARNING: Skipping invalid hostname for Caddy config: ${hostname}`)
      continue
    }
    lines.push(`        ${hostname} "localhost:${port}"`)
  }

  // Infrastructure services routed via Caddy (not direct)
  for (const svc of INFRASTRUCTURE_SERVICES) {
    if (svc.routeVia === "caddy") continue // caddy-routed services don't need map entries (they use the port directly)
    if (!isValidHostname(svc.hostname)) continue
    // Direct services also get a map entry so internal Caddy knows about them
    // (in case traffic arrives at :8444 via wildcard fallback)
...
async function main() {
  ...
  const caddyContent = generateCaddyInternal(sites, previewProxyPort)
