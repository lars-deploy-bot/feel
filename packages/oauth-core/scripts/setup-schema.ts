...
async function setupSchema() {
  ...
  try {
    ...
    await executeSql(
      "COMMENT ON TABLE lockbox.user_secrets IS 'Encrypted OAuth secrets using AES-256-GCM'"
    );
    await executeSql(
      "COMMENT ON COLUMN lockbox.user_secrets.namespace IS 'Type of secret: provider_config (tenant) or oauth_tokens (user)'"
    );
    await executeSql(
      "COMMENT ON COLUMN lockbox.user_secrets.iv IS 'Initialization vector for AES-256-GCM (12 bytes)'"
    );
    await executeSql(
      "COMMENT ON COLUMN lockbox.user_secrets.auth_tag IS 'Authentication tag for AEAD (16 bytes)'"
    );
    await executeSql(
