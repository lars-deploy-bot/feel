 ...
 * - API expected "auth_session_v4" from COOKIE_NAMES.SESSION
 ...
 * - Tests verify constant value matches expected "auth_session_v4"
 */
...
describe("Cookie Name Synchronization - Prevent Hardcoding", () => {
  ...
  it("should export COOKIE_NAMES.SESSION from @webalive/shared", () => {
    ...
    expect(COOKIE_NAMES.SESSION).toBe("auth_session_v4")
  ...
  /**
   * Verify no hardcoded "auth_session_v4" strings (must use constant)
   */
  it("should not hardcode 'auth_session' string in stream-api-client.ts", () => {
    ...
    // Should NOT have "auth_session_v4" hardcoded in the code logic
    // (Comments and strings for tests are okay, but not in Cookie header construction)
    const hardcodedAuthSession = /Cookie:\s*["`]auth_session_v4=/
    expect(codeWithoutImports).not.toMatch(hardcodedAuthSession)

    // If this test FAILS, someone hardcoded "auth_session_v4" instead of using the constant
  })
...
describe("Cookie Name Usage - Runtime Verification", () => {
  ...
  it("should construct cookie header in correct format", () => {
    ...
    expect(expectedHeader).toBe("auth_session_v4=test-jwt-123")
  ...
  it("should match cookie name expected by Next.js API routes", () => {
    ...
    // apps/web expects COOKIE_NAMES.SESSION = "auth_session_v4"
    expect(COOKIE_NAMES.SESSION).toBe("auth_session_v4")
