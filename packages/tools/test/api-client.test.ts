 ...
 * COOKIE_NAMES.SESSION ("auth_session_v4") from the shared package.
 *
 * Result: Tools sent `Cookie: session=<JWT>` but API expected `Cookie: auth_session_v4=<JWT>`
...
describe("callApi - Cookie Name Authentication (THE COOKIE NAME BUG)", () => {
   ...
   * Cookie header MUST use "auth_session_v4" not "session"
   */
  it("should use 'auth_session_v4' cookie name from shared constant, not hardcoded 'session'", async () => {
    await callApi({
    ...
    // CRITICAL: Must be "auth_session_v4" (from COOKIE_NAMES.SESSION)
    ...
    expect(options.headers.Cookie).toMatch(/^auth_session_v4=/)
  ...
  it("should use COOKIE_NAMES.SESSION constant value (auth_session)", async () => {
    ...
    // Extract cookie name from header "auth_session_v4=value"
    const cookieName = options.headers.Cookie?.split("=")[0]

    expect(cookieName).toBe(COOKIE_NAMES.SESSION)
    expect(cookieName).toBe("auth_session_v4")
  })

  /**
   * Verify cookie format is exactly "auth_session_v4=<value>" with no spaces
   */
  it("should format cookie header correctly without extra spaces", async () => {
    ...
    // Exact format check
    expect(options.headers.Cookie).toBe("auth_session_v4=jwt-token-123")
  ...
  it("should preserve cookie format with various JWT tokens", async () => {
    ...
    for (const token of testTokens) {
      ...
      const [_, options] = mockFetch.mock.calls[0]
      expect(options.headers.Cookie).toBe(`auth_session_v4=${token}`)
    }
  })
  ...
  it("should include auth_session cookie for all API endpoints", async () => {
    ...
    for (const endpoint of endpoints) {
      ...
      expect(options.headers.Cookie).toBe(`${COOKIE_NAMES.SESSION}=jwt-token-123`)
      expect(options.headers.Cookie).toMatch(/^auth_session_v4=/) // Must start with auth_session_v4=
