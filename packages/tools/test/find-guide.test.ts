...
import { type FindGuideParams, debugFindGuide, findGuide } from "../src/tools/composite/find-guide.js"
...
async function validateGuideStructure() {
	...
	for (const category of GUIDE_CATEGORIES) {
		try {
			...
			issues.push(
				`❌ Category "${category}" ERROR: ${error instanceof Error ? error.message : String(error)}`,
			)
		}
	}
