...
### `supabase--enable`
Enable Lovable Cloud (provisions Supabase backend).
...
- ✅ Secrets are encrypted and stored in Supabase
...
- ❌ CANNOT update Supabase internal secrets
...
- ❌ CANNOT delete Supabase system secrets (SUPABASE_URL, SUPABASE_ANON_KEY, etc.)
...
- `query` (required): Technical search query
  - Examples: `"React hook form validation"`, `"Supabase RLS policies examples"`
