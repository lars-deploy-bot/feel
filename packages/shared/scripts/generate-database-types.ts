 ...
 * by querying the Supabase database directly using Supabase CLI.
 */
...
const projectIdRaw = process.env.SUPABASE_PROJECT_ID
if (!projectIdRaw) {
...
const outputDir = resolve(dirname(new URL(import.meta.url).pathname), "../src/database")
const tempDir = resolve(dirname(new URL(import.meta.url).pathname), "../.tmp")
...
async function generateTypes() {
  ...
  try {
    ...
    for (const schema of schemas) {
      ...
      // Write to individual generated file
...
`
      ...
      console.log(`   Written: ${generatedFilePath}`)
    }

    // Create the main types file that imports and re-exports
...
${generatedSchemas
  .map(schema => {
    ...
    return `import { Database as ${capitalized}Database } from './${schema}.generated'`
  })
  .join("\n")}
...
export type Database = ${generatedSchemas.includes("public") ? "PublicDatabase" : generatedSchemas.includes("lockbox") ? "LockboxDatabase" : "unknown"}
`
    ...
    generatedSchemas.forEach(schema => {
      console.log(`     - ${schema}.generated.ts`)
    })
    console.log("   Main types file: database/index.ts")
  } catch (error) {
