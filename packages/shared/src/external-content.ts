 ...
 * Adapted from OpenClaw's external-content.ts
 *
 ...
 * Returns array of matched pattern sources for logging.
 */
...
export function isExternalHookSession(sessionKey: string): boolean {
  return (
    sessionKey.startsWith("hook:gmail:") || sessionKey.startsWith("hook:webhook:") || sessionKey.startsWith("hook:") // Generic hook prefix
  )
}
...
/**
 * Sanitize user input that will be interpolated into prompts.
 * Escapes characters that could be used for prompt injection.
 *
 * NOTE: This is a simple sanitizer for basic cases. For untrusted external
 * content, always use wrapExternalContent() instead which provides stronger
 * isolation through boundary markers and security warnings.
 */
export function sanitizePromptInput(input: string): string {
  // Replace characters that could be interpreted as role markers or XML tags
