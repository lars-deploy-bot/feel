...
describe("createConfig validation", () => {
  ...
  describe("fairness and CPU knobs", () => {
    it("should reject invalid fairness limits", () => {
      ...
      expect(() => createConfig({ maxWorkersPerUser: -1 })).toThrow("Invalid maxWorkersPerUser")
      expect(() => createConfig({ maxWorkersPerUser: 1.5 })).toThrow("Invalid maxWorkersPerUser")
      expect(() => createConfig({ maxWorkersPerUser: NaN })).toThrow("Invalid maxWorkersPerUser")
      expect(() => createConfig({ maxWorkersPerWorkspace: 0 })).toThrow("Invalid maxWorkersPerWorkspace")
      expect(() => createConfig({ maxWorkersPerWorkspace: -1 })).toThrow("Invalid maxWorkersPerWorkspace")
      expect(() => createConfig({ maxWorkersPerWorkspace: 1.5 })).toThrow("Invalid maxWorkersPerWorkspace")
      expect(() => createConfig({ maxWorkersPerWorkspace: NaN })).toThrow("Invalid maxWorkersPerWorkspace")
      expect(() => createConfig({ maxQueuedPerUser: 0 })).toThrow("Invalid maxQueuedPerUser")
      expect(() => createConfig({ maxQueuedPerUser: -1 })).toThrow("Invalid maxQueuedPerUser")
      expect(() => createConfig({ maxQueuedPerUser: 1.5 })).toThrow("Invalid maxQueuedPerUser")
      expect(() => createConfig({ maxQueuedPerUser: NaN })).toThrow("Invalid maxQueuedPerUser")
      expect(() => createConfig({ maxQueuedPerWorkspace: 0 })).toThrow("Invalid maxQueuedPerWorkspace")
      expect(() => createConfig({ maxQueuedPerWorkspace: -1 })).toThrow("Invalid maxQueuedPerWorkspace")
      expect(() => createConfig({ maxQueuedPerWorkspace: 1.5 })).toThrow("Invalid maxQueuedPerWorkspace")
      expect(() => createConfig({ maxQueuedPerWorkspace: NaN })).toThrow("Invalid maxQueuedPerWorkspace")
      expect(() => createConfig({ maxQueuedGlobal: 0 })).toThrow("Invalid maxQueuedGlobal")
      expect(() => createConfig({ maxQueuedGlobal: -1 })).toThrow("Invalid maxQueuedGlobal")
      expect(() => createConfig({ maxQueuedGlobal: 1.5 })).toThrow("Invalid maxQueuedGlobal")
      expect(() => createConfig({ maxQueuedGlobal: NaN })).toThrow("Invalid maxQueuedGlobal")
    })

    it("should reject invalid cpu/load knobs", () => {
      expect(() => createConfig({ workersPerCore: 0 })).toThrow("Invalid workersPerCore")
      expect(() => createConfig({ workersPerCore: -1 })).toThrow("Invalid workersPerCore")
      expect(() => createConfig({ workersPerCore: NaN })).toThrow("Invalid workersPerCore")
      ...
      expect(() => createConfig({ killGraceMs: NaN })).toThrow("Invalid killGraceMs")
      expect(() => createConfig({ orphanSweepIntervalMs: 0 })).toThrow("Invalid orphanSweepIntervalMs")
      expect(() => createConfig({ orphanSweepIntervalMs: -1 })).toThrow("Invalid orphanSweepIntervalMs")
      expect(() => createConfig({ orphanSweepIntervalMs: 1.5 })).toThrow("Invalid orphanSweepIntervalMs")
      expect(() => createConfig({ orphanSweepIntervalMs: NaN })).toThrow("Invalid orphanSweepIntervalMs")
      expect(() => createConfig({ orphanMaxAgeMs: 0 })).toThrow("Invalid orphanMaxAgeMs")
      expect(() => createConfig({ orphanMaxAgeMs: -1 })).toThrow("Invalid orphanMaxAgeMs")
