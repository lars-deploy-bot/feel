...
export const handlers = {
	text: (message: string, options?: HandlerOptions) =>
		createStreamHandler(
			new StreamBuilder().start().text(message).complete(),
			options,
		),
	...
	withThinking: (
		...
		createStreamHandler(
			new StreamBuilder().start().thinking(thinking).text(response).complete(),
			options,
		),
	...
	fileWrite: (
		...
		createStreamHandler(
			new StreamBuilder()
				.start()
				.tool("Write", { file_path: path, content }, "File written")
				.text(response)
				.complete(),
			options,
		),
	...
	conversation: (messages: string[], options?: HandlerOptions) => {
		...
		return createStreamHandler(
			builder.complete({ totalTurns: messages.length }),
			options,
		)
	},

	custom: (builder: StreamBuilder, options?: HandlerOptions) =>
	...
	maxTurns: (options?: HandlerOptions) =>
		createStreamHandler(
			new StreamBuilder()
				.start()
				.error(
					"Conversation reached maximum turn limit (25/25 turns)",
					"ERROR_MAX_TURNS",
				),
			{ ...options, fail: true },
		),
	...
	timeout: (options?: HandlerOptions) =>
		createStreamHandler(
			new StreamBuilder().start().error("Request timeout", "TIMEOUT"),
			{ ...options, fail: true },
		),
}
