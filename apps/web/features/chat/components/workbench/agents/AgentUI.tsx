...
export function StatusDot({ job }: { job: EnrichedJob }) {
  if (!job.is_active) return <div className="size-2 rounded-full bg-zinc-300 dark:bg-zinc-600 shrink-0" title="Paused" />
  ...
  if (job.last_run_status === "failure") return <div className="size-2 rounded-full bg-red-500 shrink-0" title="Failed" />
...
export function StreakBadge({ streak }: { streak: number }) {
  ...
  const hot = streak >= 10
  return (
    <span className={`inline-flex items-center gap-0.5 text-[10px] tabular-nums font-medium ${hot ? "text-orange-500 dark:text-orange-400" : "text-zinc-400 dark:text-zinc-600"}`}>
...
export function RunDots({ runs }: { runs: RecentRun[] }) {
  const dots = useMemo(() => [...runs].reverse().map(r => r.status === "success" ? "s" as const : "f" as const), [runs])
  if (dots.length === 0) return null
  return (
    <div className="flex items-center gap-[2px]">
      {dots.map((d, i) => (
        <div key={i} className={`w-[5px] h-[14px] rounded-[2px] ${d === "s" ? "bg-emerald-400/60 dark:bg-emerald-500/40" : "bg-red-400/60 dark:bg-red-500/40"}`} />
      ))}
    </div>
  )
}
...
export function TrigIcon({ type }: { type: string }) {
  const cls = "shrink-0"
  switch (type) {
    case "email": return <Mail size={11} className={cls} />
    case "webhook": return <Webhook size={11} className={cls} />
    case "one-time": return <Calendar size={11} className={cls} />
    default: return <RotateCw size={11} className={cls} />
  }
}

export function ActionButton({ onClick, loading, icon, variant, children }: {
  onClick: () => void; loading?: boolean; icon?: React.ReactNode; variant?: "default" | "warning" | "success" | "danger"; children: React.ReactNode
}) {
  const v = variant ?? "default"
  const styles = {
    default: "bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700",
    warning: "bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-500/15",
    success: "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-500/15",
...
export function AgentNav({ view, hasSelected, onNavigate }: {
  view: AgentView; hasSelected: boolean; onNavigate: (kind: AgentView["kind"]) => void
}) {
