 ...
 * and ALIVE_ENV=local set on the server. These are skipped on remote environments
...
test.describe("Protection System Verification", () => {
  ...
  // This test uses test credentials from SECURITY.LOCAL_TEST that only work with ALIVE_ENV=local
  ...
  test.skip("Allows non-Claude API calls (login, verify, etc)", async ({ page }) => {
    test.skip(!isLocalTestServer, "Requires local test server with ALIVE_ENV=local credentials")
