...
describe("SDK Tools Sync", () => {
  ...
  describe("SDK tool count", () => {
    ...
    it("should have correct tool counts in categories", () => {
      // 13 SDK allowed + 1 requiresUserApproval (ExitPlanMode) + 2 Bridge-only compat tools (Skill + BashOutput) = 16 in ALLOWED_SDK_TOOLS
      // 4 disallowed for member baseline (Task, WebSearch, TaskStop, AskUserQuestion)
      // 14 + 4 = 18 SDK total
      const allowedSDKOnly = ALLOWED_SDK_TOOLS.filter(t => !STREAM_ONLY_TOOLS.includes(t))
      expect(ALLOWED_SDK_TOOLS.length).toBe(16) // 14 SDK + 2 Bridge-only
      expect(allowedSDKOnly.length).toBe(14) // Pure SDK tools (incl. ExitPlanMode for SDK registration)
  ...
  describe("Expected tools are in correct categories", () => {
    ...
    it("should enforce planning/workflow policy", () => {
      ...
      // TodoWrite is allowed in default mode
      expect(isAllowed("TodoWrite")).toBe(true)
      // AskUserQuestion is restricted to plan/superadmin mode — use ask_clarification MCP tool in default
      expect(isDisallowed("AskUserQuestion")).toBe(true)
    })
