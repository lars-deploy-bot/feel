...
describe("claude-stream-request-matchers", () => {
  ...
  it("matches stream URL with query params by pathname", () => {
    expect(isClaudeStreamUrl("https://terminal.alive.best/api/claude/stream?requestId=abc")).toBe(true)
  })

  it("rejects stream-like URLs that are not the stream endpoint", () => {
    expect(isClaudeStreamUrl("https://terminal.alive.best/api/claude/stream/reconnect")).toBe(false)
    expect(isClaudeStreamUrl("https://terminal.alive.best/api/claude/stream/cancel")).toBe(false)
  })
  ...
  it("matches only POST requests to exact stream endpoint", () => {
    expect(isClaudeStreamPostRequest(createRequest("POST", "https://terminal.alive.best/api/claude/stream?x=1"))).toBe(
      true,
    )
    expect(isClaudeStreamPostRequest(createRequest("GET", "https://terminal.alive.best/api/claude/stream"))).toBe(false)
    expect(
      isClaudeStreamPostRequest(createRequest("POST", "https://terminal.alive.best/api/claude/stream/reconnect")),
    ).toBe(false)
  })

  it("matches only responses linked to POST stream requests and stream response URL", () => {
    const postStreamRequest = createRequest("POST", "https://terminal.alive.best/api/claude/stream")
    const getStreamRequest = createRequest("GET", "https://terminal.alive.best/api/claude/stream")
    const postReconnectRequest = createRequest("POST", "https://terminal.alive.best/api/claude/stream/reconnect")

    expect(
      isClaudeStreamPostResponse(
        createResponse(postStreamRequest, "https://terminal.alive.best/api/claude/stream?requestId=abc"),
      ),
    ).toBe(true)

    expect(
      isClaudeStreamPostResponse(createResponse(getStreamRequest, "https://terminal.alive.best/api/claude/stream")),
    ).toBe(false)

    expect(
      isClaudeStreamPostResponse(
        createResponse(postReconnectRequest, "https://terminal.alive.best/api/claude/stream/reconnect"),
      ),
    ).toBe(false)
  })
})
