...
describe("Feedback Storage", () => {
  const testFeedbackPath = join(process.cwd(), "test-feedback.json")
  ...
  it("should create empty feedback store when file doesn't exist", () => {
    const store = loadFeedback(testFeedbackPath)
  ...
  it("should save and retrieve feedback entries", () => {
    ...
    const allFeedback = getAllFeedback(testFeedbackPath)
  ...
  it("should filter feedback by workspace", () => {
    ...
    const exampleFeedback = getFeedbackByWorkspace("example.com", testFeedbackPath)
    expect(exampleFeedback).toHaveLength(2)
    expect(exampleFeedback.every(f => f.workspace === "example.com")).toBe(true)

    const otherFeedback = getFeedbackByWorkspace("other.com", testFeedbackPath)
