...
describe("Utility Functions", () => {
  ...
  describe("truncateDeep", () => {
    ...
    it("handles Error objects", () => {
      ...
      const result = truncateDeep(error)
    ...
    it("handles nested objects", () => {
      ...
      expect(result).toEqual({
        level1: { level2: { level3: { message: `${"a".repeat(200)}...[truncated 100 chars]` } } },
      })
    })
    ...
    it("handles arrays", () => {
      ...
      expect(result).toEqual([
        "short",
        `${"a".repeat(200)}...[truncated 100 chars]`,
        { message: `${"b".repeat(200)}...[truncated 100 chars]` },
      ])
    })
    ...
    it("handles circular references", () => {
      ...
      expect(result).toHaveProperty("self", "[Circular Reference]")
    })
    ...
    it("handles array circular references", () => {
      ...
      expect(result).toEqual([1, 2, 3, "[Circular Reference]"])
    })

    it("handles max depth", () => {
      const deep = { a: { b: { c: { d: { e: { f: "too deep" } } } } } }
      const result = truncateDeep(deep, 200, 3)
      expect(result).toEqual({ a: { b: { c: "[max depth reached]" } } })
    })
    ...
    it("handles objects with getters that throw", () => {
      ...
      const result = truncateDeep(obj)
      expect(result).toHaveProperty("good", "value")
      expect(result).toHaveProperty("bad", expect.stringMatching(/Error accessing property/))
    })
    ...
    it("handles mixed nested structures", () => {
      ...
      expect(result).toEqual({
        users: [
          {
            name: "Alice",
            bio: `${"a".repeat(200)}...[truncated 100 chars]`,
