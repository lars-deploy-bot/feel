/**
 * Linear Issues List Preview
 *
 * Preview for LinearIssuesResult component (list of issues).
...
export function LinearIssuesPreview() {
  ...
  const issues = FAKE_LINEAR_ISSUES.slice(0, issueCount)

  const reset = () => {
    setIssueCount(FAKE_LINEAR_ISSUES.length)
  }

  return (
    <div className="max-w-5xl">
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">LinearIssuesResult</h2>
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
          List view with filtering and sorting. Try the filter/sort controls.
        </p>
      </div>

      <div className="flex flex-col-reverse md:flex-row gap-6 md:gap-8">
        <div className="flex-1 md:max-w-2xl">
          <LinearIssuesResult data={issues} toolName="mcp__linear__list_issues" />
        </div>
        ...
        <div className="w-full md:w-48 space-y-4">
          <div className="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {[0, 1, 3, FAKE_LINEAR_ISSUES.length].map(count => (
                <button
                  key={count}
                  type="button"
                  onClick={() => setIssueCount(count)}
