...
vi.mock("@/lib/stores/featureFlagStore", () => ({
  useFeatureFlag: (flag: string) => mocks.useFeatureFlag(flag),
}))
...
describe("useChatMessaging payload safety", () => {
  ...
  it("keeps /api/claude/stream payload Anthropic-safe without local tool_result email blocks", async () => {
    const warningEvent = {
      type: "stream_message",
      requestId: "req-safe-no-block",
    ...
    await act(async () => {
      await result.current.sendMessage("hello without block")
    })
  ...
  it("keeps /api/claude/stream payload Anthropic-safe with local tool_result email block metadata present", async () => {
    ...
    await act(async () => {
      await result.current.sendMessage("hello with block")
    })
