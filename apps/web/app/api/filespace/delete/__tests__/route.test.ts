/**
 * Tests for POST /api/filespace/delete endpoint
 *
...
const TEST_WORKSPACE = path.join(tmpdir(), "filespace-delete-test-workspace")
...
function createMockRequest(body: Record<string, unknown>): NextRequest {
  return new NextRequest("http://localhost/api/filespace/delete", {
    method: "POST",
...
describe("POST /api/filespace/delete", () => {
  beforeAll(() => {
  ...
  describe("Path Traversal Prevention", () => {
    ...
    it("should block encoded traversal (%2e%2e)", async () => {
      // path.resolve handles this, but verify the endpoint doesn't decode separately
      const req = createMockRequest({
  ...
  describe("Protected Files", () => {
    ...
    it("should block deletion of package.json", async () => {
    ...
    it("should block deletion of index.js", async () => {
      const req = createMockRequest({ path: "index.js", workspace: "test" })
      const response = await POST(req)
      const data = await response.json()
    ...
    it("should block deletion of tsconfig.json", async () => {
      const req = createMockRequest({ path: "tsconfig.json", workspace: "test" })
      const response = await POST(req)
      const data = await response.json()
    ...
    it("should block deletion of bun.lockb", async () => {
      const req = createMockRequest({ path: "bun.lockb", workspace: "test" })
      const response = await POST(req)
      const data = await response.json()
  ...
  describe("Protected Directories", () => {
    it("should block deletion of node_modules", async () => {
      const req = createMockRequest({
        path: "node_modules",
        workspace: "test",
        recursive: true,
    ...
    it("should block deletion of .git", async () => {
      const req = createMockRequest({
        path: ".git",
        workspace: "test",
        recursive: true,
    ...
    it("should block deletion of .well-known", async () => {
      const req = createMockRequest({
        path: ".well-known",
        workspace: "test",
        recursive: true,
  ...
  describe("Symlink Protection", () => {
    ...
    it("should safely remove symlinks without following them", async () => {
      // delete endpoint uses lstat (not stat), so the symlink path itself
      // is within workspace. lstat + unlink removes the symlink, NOT the target.
      // This is correct behavior — the symlink file is deleted, /etc/passwd is untouched.
      const symlinkPath = path.join(TEST_WORKSPACE, "evil-symlink")
      try {
