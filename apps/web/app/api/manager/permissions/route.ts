...
export async function POST(request: NextRequest) {
	...
	try {
		...
		return NextResponse.json(
			{
				error:
					error instanceof Error ? error.message : "Failed to fix permissions",
			},
			{ status: 500 },
		)
	}
...
async function checkDomainPermissions(
	...
	try {
		...
		result.totalFiles = Number.parseInt(totalOutput.trim())
	} catch (error) {
	...
	try {
		const { stdout: rootFiles } = await execAsync(
			`find "${siteDir}" -user root -type f 2>/dev/null || true`,
		)
...
async function fixDomainPermissions(domain: string): Promise<void> {
	...
	try {
		...
		throw new Error(
			`Failed to fix permissions: ${error instanceof Error ? error.message : "Unknown error"}`,
		)
	}
}
