...
describe("POST /api/gmail/draft", () => {
  beforeEach(() => {
    ...
    mockCreateDraft.mockResolvedValue({
      data: { id: "draft_abc", message: { id: "msg_inner" } },
    })
  })
  ...
  it("saves draft and returns draftId + messageId", async () => {
    const res = await POST(createRequest(validBody))
  ...
  it("saves draft with cc and threadId", async () => {
  ...
  it("returns messageId as undefined when Gmail omits it", async () => {
    mockCreateDraft.mockResolvedValueOnce({ data: { id: "draft_1", message: null } })
    const res = await POST(createRequest(validBody))
    const data = await res.json()
    expect(data.ok).toBe(true)
    expect(data.draftId).toBe("draft_1")
    expect(data.messageId).toBeUndefined()
  })

  // --- API failure ---

  it("returns 500 when Gmail API does not return draft ID", async () => {
    mockCreateDraft.mockResolvedValueOnce({ data: { id: null } })
    const res = await POST(createRequest(validBody))
    expect(res.status).toBe(500)
    const data = await res.json()
    expect(data.ok).toBe(false)
  ...
  it("returns 500 on unexpected Gmail API error", async () => {
    mockCreateDraft.mockRejectedValueOnce(new Error("Gmail 500"))
    const res = await POST(createRequest(validBody))
    expect(res.status).toBe(500)
    const data = await res.json()
    expect(data.ok).toBe(false)
