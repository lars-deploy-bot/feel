            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 mt-2 text-sm text-slate-600">
                  <span>Credits: <span className="font-medium text-slate-900">{org.credits.toFixed(2)}</span></span>
                  <span>Members: {org.member_count}</span>
                  <span>Projects: {org.domain_count}</span>
                </div>
                <div className="text-xs text-slate-500 mt-1">Created {new Date(org.created_at).toLocaleDateString()}</div>
              </div>
                ...
                {org.domains.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {org.domains.map((domain: any) => (
                      <div key={domain.domain_id} className="px-3 py-2 bg-slate-50 rounded text-sm text-slate-700">
                ...
                {org.members.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {org.members.map((member: any) => (
                      <div key={member.user_id} className="px-3 py-2 bg-slate-50 rounded flex items-center justify-between">
