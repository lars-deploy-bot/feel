type ReadOutputProps =
	| TextFileOutputProps
	| ImageFileOutputProps
	| PDFFileOutputProps
	| NotebookFileOutputProps
...
export function ReadOutput(props: ReadOutputProps) {
				...
				<div className="space-y-4 max-h-80 overflow-auto">
					{props.pages.map((page, index) => (
						<div key={index} className="border border-black/10 p-3">
							...
							{page.text && (
								<div className="text-xs text-black/80 font-thin leading-relaxed whitespace-pre-wrap">
									{page.text}
								</div>
							)}
	...
	if ('cells' in props) {
		return (
			<div className="space-y-2">
				<div className="text-xs text-black/40 font-thin">
					notebook • {props.cells.length} cells
				</div>
						...
						<div key={index} className="border border-black/10 p-3">
							...
							{cell.outputs && cell.outputs.length > 0 && (
								<div className="mt-2 pt-2 border-t border-black/5">
									<pre className="text-xs text-black/60 font-diatype-mono">
