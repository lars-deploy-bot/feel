...
export function StripeSubscriptionsOutput({ subscriptions }: StripeSubscriptionsOutputProps) {
  if (!subscriptions || subscriptions.length === 0) {
    return <div className="text-[13px] text-black/50 dark:text-white/50 py-2">No subscriptions found</div>
  }
      ...
      <table className="w-full text-xs">
        <thead className="bg-black/[0.02] dark:bg-white/[0.02] sticky top-0 z-10">
          <tr className="border-b border-black/[0.06] dark:border-white/[0.08]">
            ...
            <th className="text-left px-3 py-2 font-medium text-black/50 dark:text-white/50">Plan</th>
          ...
          {subscriptions.map((sub, index) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-3 py-2">
                  {nextBilling ? (
                    <div className="text-black/60 dark:text-white/60">{formatNextBilling(nextBilling)}</div>
                  ) : (
                    <span className="text-black/30 dark:text-white/30">—</span>
                  )}
                </td>
