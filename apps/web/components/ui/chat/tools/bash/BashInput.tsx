...
export function BashInput({ command, description, timeout, run_in_background }: BashInputProps) {
  return (
    <div className="space-y-2">
      <div className="text-xs text-black/60 font-diatype-mono">{command}</div>
      {description && <div className="text-xs text-black/40 font-thin">{description}</div>}
      {(timeout || run_in_background) && (
        <div className="text-xs text-black/30 font-thin">
