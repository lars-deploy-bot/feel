        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-sm font-medium ${isSelected ? "" : ""}`}>{preset.label}</p>
              {showDescription && (
                <p
                  className={`text-xs mt-0.5 ${isSelected ? "text-white/70 dark:text-black/70" : "text-black/50 dark:text-white/50"}`}
                >
                  {preset.description}
                </p>
              )}
