...
import type { AriaRole, RoleRefMap, RoleSnapshotOptions, RoleSnapshotStats, SnapshotResult } from "./types.js"
...
export function requireRef(value: unknown): string {
  ...
  if (!roleRef) {
    throw new UserInputError(raw ? `Invalid ref format: "${raw}"` : "ref is required")
  }
  return roleRef
}

/** Type guard: validates a role string is a known AriaRole (in INTERACTIVE_ROLES or CONTENT_ROLES). */
...
export function refLocator(page: Page, ref: string, roleRefs: RoleRefMap | undefined) {
  ...
  if (!isAriaRole(info.role)) {
    throw new UserInputError(`Unknown role "${info.role}" for ref "${normalized}".`)
  }

  const locator = info.name ? page.getByRole(info.role, { name: info.name, exact: true }) : page.getByRole(info.role)
...
/** Minimal shape of a CDP Accessibility tree node, compatible with Protocol.Accessibility.AXNode. */
interface CDPAXNode {
  nodeId: string
  role?: { value?: string }
  name?: { value?: string }
