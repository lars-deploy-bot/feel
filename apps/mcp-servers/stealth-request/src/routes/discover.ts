...
function buildApiSchema(config: DiscoverConfig) {
  const { port, publicUrl } = config
  return {
    ...
    endpoints: [
      {
        ...
        example: { curl: `curl ${publicUrl}/health` },
      },
      {
        method: "GET",
        path: "/discover",
      ...
      },
      {
        ...
        params: {
          ...
          followPagination: {
            ...
            required: false,
            description:
              "Click a 'next' selector repeatedly, collecting content from each page (GET only, max 50 pages)",
          },
        ...
        example: {
          curl: `curl -X POST ${publicUrl}/fetch -H "Content-Type: application/json" -d '{"url": "https://example.com", "waitFor": "h1", "format": "markdown"}'`,
        },
      },
      {
        method: "POST",
        ...
        params: {
          requests: {
            type: "array of /fetch params",
            required: true,
            description: "Array of request objects (max 50)",
          },
          concurrency: {
            type: "number",
            required: false,
            default: 5,
            description: "Max concurrent browser instances",
          },
        },
        response: {
          success: "boolean",
        ...
        example: {
          curl: `curl -X POST ${publicUrl}/fetch-batch -H "Content-Type: application/json" -d '{"requests": [{"url": "https://example.com"}, {"url": "https://httpbin.org/html"}], "concurrency": 2}'`,
        },
      },
      {
        method: "POST",
        ...
        example: {
          curl: `curl -X POST ${publicUrl}/detect-framework -H "Content-Type: application/json" -d '{"url": "https://vercel.com"}'`,
        },
      },
      {
        method: "POST",
        ...
        example: {
          curl: `curl -X POST ${publicUrl}/discover-subdomains -H "Content-Type: application/json" -d '{"domain": "example.com"}'`,
        },
      },
      {
        method: "POST",
        ...
        example: {
          curl: `curl -X POST ${publicUrl}/dns-records -H "Content-Type: application/json" -d '{"domain": "example.com"}'`,
        },
      },
      {
        method: "POST",
        ...
        example: {
          curl: `curl -X POST ${publicUrl}/whois -H "Content-Type: application/json" -d '{"domain": "example.com"}'`,
        },
      },
      {
        method: "POST",
        ...
        example: {
          curl: `curl -X POST ${publicUrl}/port-scan -H "Content-Type: application/json" -d '{"host": "example.com"}'`,
        },
      },
      {
        method: "POST",
        ...
        example: {
          curl: `curl -X POST ${publicUrl}/analyze-js -H "Content-Type: application/json" -d '{"url": "https://vercel.com"}'`,
        },
      },
    ],
  }
...
export function registerDiscoverRoutes(router: Router, config: DiscoverConfig) {
  router.get("/discover", (_req: Request, res: Response) => {
    res.json(buildApiSchema(config))
  })
}
