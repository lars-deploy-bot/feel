...
// All tool definitions — typed via MCP SDK's Tool interface (eliminates need for `as const`)
const allTools: Tool[] = [
  {
    name: "compose_email",
    description:
  ...
  try {
    switch (toolName) {
      case "compose_email": {
        ...
        const invalidTo = toArray.filter((addr: string) => !looksLikeEmail(addr))
        ...
        const invalidCc = ccArray?.filter((addr: string) => !looksLikeEmail(addr)) ?? []
        const invalidBcc = bccArray?.filter((addr: string) => !looksLikeEmail(addr)) ?? []
        if (invalidCc.length > 0 || invalidBcc.length > 0) {
          return {
            content: [
              {
                type: "text",
                text: `Error: Invalid email address(es): ${[...invalidCc, ...invalidBcc].join(", ")}`,
              },
            ],
            isError: true,
          }
