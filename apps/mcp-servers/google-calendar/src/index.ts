 ...
 *
 * MCP-only server for Google Calendar functionality.
 * Accepts Bearer tokens via HTTP Authorization header.
 *
 * REST endpoints (create, update, delete events) are handled by Next.js API routes,
 * not this server. This keeps concerns separated:
 * - MCP server: Claude tool calls (read, compose, propose)
 * - Next.js API: User UI actions (Create/Update/Delete buttons)
 */
...
/**
 * Create MCP server with Calendar client injected
 */
function createServer(accessToken?: string) {
  ...
  server.setRequestHandler(CallToolRequestSchema, async request => {
    ...
    if (!accessToken) {
      return {
        content: [
          {
            type: "text",
            text: "Error: No access token provided. Include Authorization: Bearer <token> header.",
          },
        ],
        isError: true,
      }
    ...
    return await executeTool(calendar, name, args || {})
  })
...
async function startStdioServer() {
  ...
  console.error("Google Calendar MCP Server started (stdio)")
}
...
async function startHttpServer() {
  ...
  app.get("/health", (_req, res) => {
    res.json({ status: "ok", server: "google-calendar" })
  })
  ...
  app.listen(PORT, () => {
    console.error(`Google Calendar MCP Server running on http://localhost:${PORT}/mcp`)
