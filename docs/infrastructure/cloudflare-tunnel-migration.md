...
- Validates the Caddy config with `caddy validate` before applying
- Reloads Caddy automatically (with rollback on failure)
- Includes infrastructure services (widget, mg, oc, services, dl1) from the registry in `packages/shared/src/infrastructure-services.ts`
- Aborts if fewer than 10 sites are returned from DB (safety guard against catastrophic route wipe)

### 7. Add wildcard + dl1 + service routes

The sync now includes infrastructure services automatically, but still misses:
...
**`/etc/caddy/Caddyfile.internal`** — AUTO-GENERATED by `packages/tunnel/src/sync.ts`. Do not edit manually. Internal :8444 for tunnel-backed sites:
```caddy
:8444 {
