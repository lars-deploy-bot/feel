...
--config mcp_servers.alive-tools.args=["/srv/workspace/abc"]
```
...
${s.env ? Object.entries(s.env).map(([k,v]) => `env.${k} = "${v}"`).join('\n') : ''}
`).join('\n');
  
  await writeFile(path.join(sessionDir, '.codex', 'config.toml'), toml);
}
```
