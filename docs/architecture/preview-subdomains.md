...
Browser (iframe in Sandbox.tsx)
  ↓
https://protino-alive-best.preview.terminal.goalive.nl/
  ↓
...
**Wildcard DNS record:**
```
*.preview.terminal.goalive.nl → Server IP
```

This allows any `<label>.preview.terminal.goalive.nl` to resolve to the Caddy server.
...
- **`Content-Security-Policy`**: Restricts embedding to terminal.goalive.nl domains only
...
// Example: https://protino-alive-best.preview.terminal.goalive.nl/
```
...
function getPreviewUrl(workspace: string): string {
  ...
  return `https://${label}.preview.terminal.goalive.nl/`
}
```

**Domain conversion examples:**
- `protino.alive.best` → `protino-alive-best.preview.terminal.goalive.nl`
- `demo.goalive.nl` → `demo-goalive-nl.preview.terminal.goalive.nl`
- `kranazilie.nl` → `kranazilie-nl.preview.terminal.goalive.nl`
...
1. User loads sandbox iframe
   ↓
2. Browser requests https://protino-alive-best.preview.terminal.goalive.nl/
   ↓
...
### 1. DNS Resolution
```bash
dig protino-alive-best.preview.terminal.goalive.nl
...
dig protino-alive-best.preview.terminal.goalive.nl
```
**Ensure wildcard record exists:** `*.preview.terminal.goalive.nl → Server IP`
...
// https://protino-alive-best.preview.terminal.goalive.nl/?token=<jwt>
...
useEffect(() => {
  const ws = new WebSocket('wss://protino-alive-best.preview.terminal.goalive.nl/__logs')
  ws.onmessage = (event) => {
