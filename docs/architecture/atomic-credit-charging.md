...
```typescript
// ✅ GOOD: Atomic deduction via RPC
export async function chargeTokensFromCredits(
  domain: string,
...
### Updated Credit Charging Function
...
3. Next request will be blocked at upfront check (line 158-173 in `route.ts`)
...
Changes made:
- ✅ `apps/web/lib/credits/supabase-credits.ts` - Uses RPC instead of manual update
- ✅ `apps/web/lib/stream/ndjson-stream-handler.ts` - Returns structured result
...
```bash
bun test lib/credits/__tests__/atomic-deduction.test.ts
...
```bash
cd apps/web && bun test lib/credits/__tests__/atomic-deduction.integration.test.ts
...
- **TypeScript Implementation**: `apps/web/lib/credits/supabase-credits.ts:188-247`
- **Stream Handler**: `apps/web/lib/stream/ndjson-stream-handler.ts:99-135`
- **Architecture Analysis**: `docs/architecture/STREAM_ORCHESTRATOR_ANALYSIS.md:599-628`
