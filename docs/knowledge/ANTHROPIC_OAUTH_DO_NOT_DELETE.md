...
│      │        │                                                      │
│      │        ▼                                                      │
│      │    Worker Pool Manager ──► IPC payload                        │
│      │        │                                                      │
│      │        ▼                                                      │
│      │    worker-entry.mjs                                           │
│      │        │                                                      │
│      │        ▼                                                      │
│      │    prepareRequestEnv() ──► CLAUDE_CODE_OAUTH_TOKEN env var    │
│      │        │                                                      │
│      │        ▼                                                      │
...
| Per-request runners | `CLAUDE_CONFIG_DIR` env var | Points to `~/.claude` |

### Permission requirements

| Path | Required perms | Why |
|------|---------------|-----|
| `/root` | `711` (drwx--x--x) | Workers traverse to `.claude` after UID drop |
| `/root/.claude` | `711` (drwx--x--x) | Workers traverse to credentials file |
| `/root/.claude/.credentials.json` | `644` (-rw-r--r--) | Workers read after UID drop |

**Problem:** CLI writes `0o600`. Workers can't read that. The `claude-credentials-fix` systemd path unit (if installed) fixes this automatically. Otherwise, manually `chmod 644` after `/login`.
