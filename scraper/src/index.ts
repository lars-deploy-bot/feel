...
// ---------------------------------------------------------------------------
const STALE_DIR_PATTERNS = ["puppeteer_dev_profile-", ".org.chromium.Chromium."]
const STALE_THRESHOLD_MS = 10 * 60 * 1000 // 10 minutes

/** Remove temp dirs matching known Puppeteer patterns that are older than threshold. */
export function cleanupStaleTempDirs(): number {
  const tmp = tmpdir()
  let cleaned = 0
  try {
    ...
    for (const entry of entries) {
      if (!STALE_DIR_PATTERNS.some(p => entry.startsWith(p))) continue
    ...
    // tmpdir read failed — non-fatal
  }
  if (cleaned > 0) {
    console.log(`[cleanup] Removed ${cleaned} stale Puppeteer temp dirs`)
  }
  return cleaned
}

// Run on module load (server startup) and every 5 minutes
